...
# A11.  ``MLX_VLM_GLM5_PREFILL_TAIL_MIN`` is an ABSOLUTE token count sized for
# the shipped ``C = 8192`` (1024 = C/8), and ``next_prefill_chunk`` clamps it to
# ``step`` -- so at this file's ``C = 8`` the shipped default makes the merge
# window a WHOLE chunk wide instead of an eighth of one, and the 40-token prompt
# every test here is written around falls inside it (the last pipelined chunk
# has 15 columns left, and 8 < 15 < 8 + 8).  That is a fixture artifact of the
# clamp, not the geometry under test: at the served size a ``T = qC`` prompt is
# nowhere near the window.  So the file pins the RATIO instead of the constant,
# and ``TestTailMergeTimesPP`` below turns the merge on deliberately, at the
# remainders that fire it, and checks what it does.
TAIL_MIN = 4
...
#
#   (a) MLX_VLM_GLM5_PREFILL_LOGITS_KEEP -- the last prefill forward slices the
#       hidden to one row BEFORE the vocab projection;
...
def _merge_batch(lm, *, rows=None, vault=None, rungs=None, apc=None, checkpoint=None):
    ...
    if vault is not None:
        batch._vault = vault
        meta["vault_rungs"] = list(rungs or [])
    if apc is not None:
        batch._apc_manager = apc
        batch._apc_mode = "exact"
        meta["checkpoint_len"] = int(checkpoint)
        meta["extra_hash"] = 0
    if vault is not None or apc is not None:
        batch._apc_meta = [meta]
    return batch


def _both_arms(monkeypatch, make):
    """Run ``make()`` pipelined and single-box; return both results and the peer."""
    pr.METRICS.reset()
    made = _arm(monkeypatch)
    pp = _drain(make())
    with _pipeline_off():
...
def test_with_the_merge_off_the_schedule_is_the_full_k(monkeypatch):
    ...
    pp, ref, made = _both_arms(monkeypatch, lambda: _merge_batch(_lm()))
    assert made[0].chunks == [STEP] * MERGE_FULL_CHUNKS
    assert pp[2] == ref[2] == [STEP] * MERGE_FULL_CHUNKS
...
def test_the_merged_remainder_still_yields_to_a_vault_rung(monkeypatch, column):
    """The clamp runs AFTER the merge, so it can only shorten a grown chunk.

    A rung at 36 (or 34) lands strictly inside the head's merged remainder
    ``[32, 41)``.  Merging must not carry the prefill past it: the loop clamps
    to the rung, stores a COMPLETE cache there -- all layers are home after
    ``finalize`` -- and the entry is the single-box entry, array for array.
    """
    vaults = {}

    def make(name):
        def build():
            vaults[name] = _FakeVault()
            return _merge_batch(_lm(), vault=vaults[name], rungs=[column])
    ...
    want, n_want = _fragments_digest(vaults["ref"].inserts[0]["fragments"])
    assert n_got == n_want > 0 and got == want


def test_the_merged_remainder_still_yields_to_the_apc_exact_column(monkeypatch):
    """Same clamp, on the column that cannot move.

    A vault rung may be collapsed onto another depth; an APC exact entry is
    keyed on ``full_input_ids[:checkpoint_len]`` and can only be stored at
    exactly that length.  It falls in the merged remainder here, so what
    protects it is the clamp -- and the entry it produces is byte-equal to the
    single-box one.
    """
    column = 36
    managers = {}
    ...
    def make(name):
        def build():
            managers[name] = _apc_manager()
            return _merge_batch(_lm(), apc=managers[name], checkpoint=column)
...
def test_the_head_final_forward_keeps_one_row_exactly_as_one_box_does(monkeypatch):
    """L35(a) x PP: the forward after ``finalize`` is the single-box forward.

    On the ``r = 0`` fixture the head's remainder is a whole chunk wide, so the
    projection this forward would otherwise build is [1, 8, vocab] for one row
    -- and ``prefill_logits_keep_kwargs`` narrows it in BOTH arms, with the same
    kwarg, at the same width.  (At width 1 the kwarg is withheld, which is why
    the merge geometry above cannot pin this.)
    """
    seen = {}
    ...
    def spy_arm(name):
        lm = _lm()
        calls = []
        orig = type(lm).__call__

        def spy(self, *a, **kw):
            calls.append(dict(kw))
            return orig(self, *a, **kw)

        monkeypatch.setattr(type(lm), "__call__", spy)
        seen[name] = calls
        return lm

    _arm(monkeypatch)
    pp = _drain(_batch(spy_arm("pp")))
    with _pipeline_off():
        ref = _drain(_batch(spy_arm("ref")))
    monkeypatch.undo()

    for name in ("pp", "ref"):
        final = seen[name][-1]
        assert final.get("num_logits_to_keep") == 1, name
        assert "n_to_process" not in final, "that is the post-loop forward"
    chunk_kwargs = [c for c in seen["pp"] if "n_to_process" in c]
    assert chunk_kwargs and all(
        "num_logits_to_keep" not in c for c in chunk_kwargs
    ), "the chunk loop drops its logits instead; the kwarg would change its call"
    assert pp[0] == ref[0] and mx.array_equal(pp[1], ref[1])


def test_the_fused_hc_kernels_are_on_by_default_and_the_double_still_matches(
    monkeypatch,
):
    """L34 x PP: same source on both boxes, so the boundary tensor is the same.

    ``pipeline_forward`` runs ``self.layers[i]`` -- the same modules
    ``__call__`` runs -- so the fused mHC expand/pre-norm kernels are reached
    through the same gate on the head, on the tail and on one box, and the
    boundary tensor cannot be produced by a different path than the single-box
    activation at that layer.  What is asserted here is the part that is
    assertable without a peer and without a GPU: the flags are ON with no
    environment, and the loopback double still lands on the single-box cache.

    NOT asserted: the kernels themselves.  ``_metal_ok()`` is False on CPU and
    the expand kernel additionally needs >= 512 rows, so this arm exercises the
    fallback bodies.  The kernel-vs-eager identity is
    ``test_glm5_next_hc_fused.py``'s job (47 tests, GPU-gated).
    """
    from mlx_vlm.models.deepseek_v4 import hyper_connection as hc

    for name in (
        "MLX_VLM_GLM5_HC_FUSED",
        "MLX_VLM_GLM5_HC_PRENORM_FUSED",
        "MLX_VLM_GLM5_PREFILL_LOGITS_KEEP",
        "MLX_VLM_GLM5_PREFILL_TAIL_MERGE",
    ):
        monkeypatch.delenv(name, raising=False)
    hc._reset_flag_cache()
    try:
        assert hc._hc_fused_enabled() is True
        assert hc._hc_prenorm_fused_enabled() is True
        pp, ref, made = _both_arms(monkeypatch, lambda: _batch(_lm()))
    finally:
        hc._reset_flag_cache()
    assert made[0].chunks == PIPELINED_CHUNKS
    assert pp[0] == ref[0] and mx.array_equal(pp[1], ref[1])


# --- the byte pins -----------------------------------------------------------
#
# ``A6634A75_COLD_CACHE`` is the cache a cold 40-token prefill left at
# ``a6634a75``.  Under the unified defaults the SAME fixture still lands on it
# (this file pins ``TAIL_MIN`` to the served ratio, so the merge does not fire
# at ``r = 0``, and L35(a) moves logits and not cache), which is why the tests
# above can keep comparing against it.  What the shipped ``TAIL_MIN = 1024``
# does at ``C = 8`` -- the clamp makes the window a whole chunk wide -- is a
# different prefill, and it gets its own pin so that "the defaults moved the
# prefill" is a visible fact rather than an invisible one.

UNIFIED_DEFAULT_COLD_CACHE = (
    "daaf98ea1529793af1ab0cacd3682f10ef5fe150d0cacf8af56d66f6c008be9e"
)


def test_the_cold_prefill_at_the_shipped_tail_min_is_pinned(monkeypatch):
    """The unified-default cold prefill: one merged chunk, and a new digest.

    ``TAIL_MIN`` back to its shipped 1024, which ``next_prefill_chunk`` clamps
    to ``step`` -- so at this fixture's ``C = 8`` the last chunk swallows the
    whole tail (``[8, 8, 8, 15]``) and the final forward is one column.  The
    digest is therefore NOT ``A6634A75_COLD_CACHE``, and that is the point: the
    L35(b) default is a prefill change, licensed by the I1437 rail (text sha +
    acceptance) and not by identity.
    """
    monkeypatch.delenv("MLX_VLM_PIPELINE_HOSTS", raising=False)
    ...
    digest, _, steps = _drain(_batch(_lm()))
    assert steps == [STEP, STEP, STEP, 2 * STEP - 1]
    assert digest == UNIFIED_DEFAULT_COLD_CACHE
    assert digest != A6634A75_COLD_CACHE


def test_all_three_levers_off_restores_the_a6634a75_digest(monkeypatch):
    """The revert knob, end to end: the three defaults, off, on one prefill.

    ``LOGITS_KEEP=0 TAIL_MERGE=0 HC_FUSED=0`` is what an operator sets when the
    L40 KL gate fails, so it is pinned as a set and not one lever at a time.
    (On CPU the HC lever is inert -- ``_metal_ok()`` is False -- so this arm
    proves the other two and carries the third as a smoke.)
    """
    from mlx_vlm.models.deepseek_v4 import hyper_connection as hc
