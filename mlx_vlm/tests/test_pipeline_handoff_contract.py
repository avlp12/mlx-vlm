...
def test_generate_step_keeps_the_pipelined_chunk_at_exactly_C(monkeypatch, pipelined):
    """A11.  L35(b) must not grow a chunk the envelope fixed.

    Five tokens at ``step = 3``: the loop has 4 columns to place, which is
    inside the merge window (``3 < 4 < 3 + tail_min``), so single-box this is
    ONE chunk of 4.  ``PrefillEnvelope.create`` would have derived ``(3, 1)``
    from the same prompt, and ``prefill_chunk`` raises on any other width -- and
    on THIS call site a raise is not a fallback, it is the request's exception
    (the ``try`` here only closes the peer).  So with a peer open the widths are
    ``3, 1``; with none they are the merged ``4``.
    """
    import sys
    from unittest.mock import MagicMock
    from mlx_vlm import pipeline_runtime
    ...
    embedding.to_dict.return_value = {}
    model.get_input_embeddings.return_value = embedding
    # ArraysCache, not a KVCache: this model is a mock and writes nothing, and
    # the single-box arm evaluates ``[c.state for c in prompt_cache]`` after
    # every chunk -- an unwritten KVCache raises there for reasons that have
    # nothing to do with chunk widths.
    cold = [ArraysCache(2), ArraysCache(2)]
    monkeypatch.setattr(
        generate_module.cache, "make_prompt_cache", lambda *a, **k: cold
    )
    monkeypatch.setattr(generate_module, "make_logits_processors", lambda *a, **k: [])
    ...
    pipeline.local_caches.return_value = []
    monkeypatch.setattr(
        pipeline_runtime, "maybe_open_pipeline", MagicMock(return_value=pipeline)
    )
    ids = mx.array([[1, 2, 3, 4, 5]])
    gen = ar.generate_step(ids, model, None, None, max_tokens=1, prefill_step_size=3)
    next(gen)
    gen.close()

    if pipelined:
        widths = [
            int(c.args[1].shape[1]) for c in pipeline.prefill_chunk.call_args_list
        ]
        assert widths == [3, 1], "the envelope's own arithmetic"
        envelope = PrefillEnvelope.create(
            model_sha256="a" * 64,
            source_revision="b" * 40,
            split=1,
            n_layers=2,
            input_ids=ids[:, :-1],  # what ``PipelineClient.begin`` hashes
            chunk=3,
        )
        assert list(envelope.chunks) == widths
        first = model.language_model.call_args_list[0]
        assert int(first.args[0].shape[1]) == 1, (
            "the loop consumed 3 + 1 and left exactly the last token for the "
            "post-finalize forward"
        )
    else:
        pipeline.prefill_chunk.assert_not_called()
        widths = [
            int(c.kwargs["n_to_process"])
            for c in model.language_model.call_args_list
            if "n_to_process" in c.kwargs
        ]
        assert widths == [4], "single-box: L35(b) merges the tail"
